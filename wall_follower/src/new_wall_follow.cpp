...
double front = 0.0;
double front_wall_1 = 0.0;
double front_wall_2 = 0.0;
...
public:
    ...
    void encoders_feedback(const ras_arduino_msgs::Encoders& msg)
    {
        ...
        estimated_w_left = (delta_enc1*2.0*PI* control_frequency)/ticks;
        estimated_w_right = (delta_enc2*2.0*PI* control_frequency)/ticks;
        estimated_linear_vel = (estimated_w_right + estimated_w_left) * wheel_radius_ / 2.0 ;
        ...
        if(buffer_count > 6)
        {
            ...
            if(wall_following)
            {
                ...
                if(left_dist <= right_dist)
                {
                    ...
                    //Check Front wall
                    if(front <= MIN_DIST_FRONT) {
                        std::cout << front << " - Front Wall \n";
                        linear_vel = 0.0;
                        angular_vel = 0.0;
            ...
            {
                if(delta_enc1 > 15 | delta_enc2 > 15)
                {
                    stopRobo();
                }else
                {
    ...
    bool turnRobo()
    {
        ...
        angular_vel = beta * turn_direction * (PI/4.0 - theta);
        rotate_counter += 1;

        last_time = current_time;
        if (fabs(theta) >= PI/8.0 || rotate_counter == 250)
        {
